...
BEGIN
  ...
  -- Réinitialiser si on est dans une nouvelle semaine
...
-- 9. Fonction pour réinitialiser les appels de tous les utilisateurs (exécutée chaque lundi)
...
COMMENT ON TABLE user_call_limits IS 'Stocke les limites d''appels hebdomadaires pour chaque utilisateur (2 appels/semaine, reset chaque lundi)';
COMMENT ON TABLE calcom_bookings IS 'Historique des réservations Cal.com synchronisées';
COMMENT ON FUNCTION use_call IS 'Décrémente le nombre d''appels disponibles pour un utilisateur';
COMMENT ON FUNCTION get_user_call_limits IS 'Récupère les limites d''appels d''un utilisateur (avec auto-reset si nouvelle semaine)';
COMMENT ON FUNCTION reset_weekly_call_limits IS 'Réinitialise les appels de tous les utilisateurs (à exécuter chaque lundi via cron)';

-- 14. Créer un cron job pour réinitialiser les appels chaque lundi à 00:00
-- Note: Nécessite l'extension pg_cron (à activer dans Supabase Dashboard)
-- CREATE EXTENSION IF NOT EXISTS pg_cron;
-- SELECT cron.schedule('reset-weekly-calls', '0 0 * * 1', 'SELECT reset_weekly_call_limits()');
